...
DECLARE
  ...
  v_comp RECORD; v_auto_cerradas integer := 0;
BEGIN
  ...
  -- Caso especial: solicitud apuntando a sesión que ya fue cancelada previamente.
  -- Solo cerramos la solicitud como obsoleta, sin tocar inventario.
  IF v_session.estado = 'cancelado' AND p_solicitud_id IS NOT NULL THEN
    ...
    VALUES (v_user_id,
      'cerrar_solicitud_obsoleta',
      format('Solicitud cerrada (sesión ya cancelada) — Cliente: %s', v_session.cliente_nombre),
      jsonb_build_object('session_id', p_session_id, 'solicitud_id', p_solicitud_id,
        'cliente_nombre', v_session.cliente_nombre, 'sesion_estado', v_session.estado,
        'motivo', p_motivo));

    RETURN json_build_object('ok', true, 'session_id', p_session_id,
      'ya_cancelada', true, 'mermas_creadas', 0, 'entregados_count', 0);
  END IF;
  ...
  ELSE
    -- Auto-cierre de solicitudes huérfanas cuando se cancela directo
    UPDATE public.solicitudes_cancelacion_sesiones
      SET estado = 'aprobada', revisado_por = v_user_id,
          motivo_rechazo = COALESCE(motivo_rechazo, 'Auto-cerrada: sesión cancelada directamente por administrador')
      WHERE session_id = p_session_id AND estado = 'pendiente';
    GET DIAGNOSTICS v_auto_cerradas = ROW_COUNT;
  END IF;
  ...
  v_descripcion_audit := CASE
    ...
    ELSE
      format('Cancelación directa — Cliente: %s · Entregados: %s item(s) · %s merma(s) · %s solicitud(es) auto-cerradas',
             v_session.cliente_nombre, v_total_entregados, v_mermas_creadas, v_auto_cerradas)
  END;
  ...
  VALUES (v_user_id,
    ...
    jsonb_build_object('session_id', p_session_id, 'area_id', v_session.area_id,
      ...
      'solicitudes_auto_cerradas', v_auto_cerradas,
...
-- Backfill: cerrar solicitudes huérfanas existentes
UPDATE public.solicitudes_cancelacion_sesiones s
   SET estado = 'aprobada',
       motivo_rechazo = COALESCE(s.motivo_rechazo, 'Cierre retroactivo: sesión ya estaba cancelada')
  FROM public.coworking_sessions c
