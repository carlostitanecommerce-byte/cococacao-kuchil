-- ================================================================
-- Migración 1/2: fix_cancelar_sesion_coworking_paquetes
-- ================================================================
...
BEGIN
  ...
  UPDATE public.coworking_sessions SET estado='cancelado', monto_acumulado=0, fecha_salida_real=now() WHERE id = p_session_id;
...
-- ================================================================
-- Migración 2/2: add_fecha_descuento_to_mermas
-- ================================================================
...
BEGIN
  ...
  IF NOT (public.has_role(v_user,'administrador'::app_role) OR public.has_role(v_user,'supervisor'::app_role) OR public.has_role(v_user,'barista'::app_role)) THEN
    RAISE EXCEPTION 'Permisos insuficientes para resolver cancelación' USING ERRCODE='42501'; END IF;
  IF p_decision NOT IN ('retornado_stock','merma','rechazado') THEN RAISE EXCEPTION 'Decisión inválida: %', p_decision; END IF;
  ...
  IF v_cancel.estado <> 'pendiente_decision' THEN RAISE EXCEPTION 'Esta solicitud ya fue resuelta (estado: %)', v_cancel.estado; END IF;

  IF p_decision='rechazado' THEN
    IF v_cancel.kds_item_id IS NOT NULL THEN
      UPDATE public.kds_order_items SET cancel_qty=GREATEST(0,cancel_qty - v_cancel.cantidad),
        cancel_requested=(GREATEST(0,cancel_qty - v_cancel.cantidad) > 0) WHERE id=v_cancel.kds_item_id;
    END IF;
    UPDATE public.cancelaciones_items_sesion SET estado='rechazado', decidido_por=v_user, decided_at=now(), notas_cocina=p_notas WHERE id=p_cancelacion_id;
    INSERT INTO public.audit_logs (user_id, accion, descripcion, metadata)
  ...
  IF v_cancel.detalle_id IS NOT NULL THEN
    SELECT * INTO v_dv FROM public.detalle_ventas WHERE id=v_cancel.detalle_id AND venta_id IS NULL FOR UPDATE;
  END IF;
  ...
  IF v_dv.id IS NOT NULL THEN
    ...
    IF v_dv.producto_id IS NOT NULL AND v_dv.paquete_id IS NULL THEN
      FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=v_dv.producto_id ORDER BY r.insumo_id LOOP
        IF p_decision='retornado_stock' THEN
          UPDATE public.insumos SET stock_actual=stock_actual+(v_receta.cantidad_necesaria*v_cancel.cantidad) WHERE id=v_receta.insumo_id;
        ELSE
          ...
          VALUES (v_receta.insumo_id, v_receta.cantidad_necesaria*v_cancel.cantidad,
            format('Cancelación coworking — %s ×%s (sesión %s)', v_cancel.nombre_producto, v_cancel.cantidad, v_dv.coworking_session_id),
    ...
    ELSIF v_dv.paquete_id IS NOT NULL THEN
      FOR v_comp IN SELECT producto_id AS pid, cantidad AS qty FROM public.paquete_componentes WHERE paquete_id=v_dv.paquete_id ORDER BY producto_id LOOP
        FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=v_comp.pid ORDER BY r.insumo_id LOOP
          IF p_decision='retornado_stock' THEN
            UPDATE public.insumos SET stock_actual=stock_actual+(v_receta.cantidad_necesaria*v_comp.qty*v_cancel.cantidad) WHERE id=v_receta.insumo_id;
          ELSE
            INSERT INTO public.mermas (insumo_id, cantidad, motivo, usuario_id, fecha_descuento)
            VALUES (v_receta.insumo_id, v_receta.cantidad_necesaria*v_comp.qty*v_cancel.cantidad,
              format('Cancelación coworking — paquete %s ×%s (sesión %s)', v_cancel.nombre_producto, v_cancel.cantidad, v_dv.coworking_session_id),
              v_user, COALESCE(v_dv.created_at, now()));
            v_mermas := v_mermas + 1;
          END IF;
        END LOOP;
    ...
    ELSE
      UPDATE public.detalle_ventas SET cantidad=v_nueva_cantidad, subtotal=v_dv.precio_unitario*v_nueva_cantidad WHERE id=v_dv.id;
    END IF;
  END IF;

  IF v_cancel.kds_item_id IS NOT NULL THEN
    SELECT cantidad, cancel_qty INTO v_total_kds, v_cancelados_kds FROM public.kds_order_items WHERE id=v_cancel.kds_item_id FOR UPDATE;
    IF v_total_kds IS NOT NULL THEN
      ...
      ELSE
        UPDATE public.kds_order_items SET cantidad=v_total_kds - v_cancel.cantidad,
  ...
  UPDATE public.cancelaciones_items_sesion
    SET estado=CASE WHEN p_decision='merma' THEN 'merma'::cancelacion_item_estado ELSE 'retornado_stock'::cancelacion_item_estado END,
  ...
  VALUES (v_user,'resolver_cancelacion_item_sesion',
    ...
    jsonb_build_object('cancelacion_id',p_cancelacion_id,'decision',p_decision,'mermas_creadas',v_mermas,'transaccional',true));
...
-- recalcular_amenities_pax redefinición (usa fecha_descuento en mermas)
CREATE OR REPLACE FUNCTION public.recalcular_amenities_pax(p_session_id uuid, p_new_pax integer)
RETURNS json LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  v_user uuid := auth.uid();
  v_session RECORD; v_amenities jsonb; v_amenity jsonb;
  v_producto_id uuid; v_cant_incluida integer; v_nombre text;
  v_existing_id uuid; v_old_qty integer; v_new_qty integer; v_delta integer;
  v_receta RECORD; v_stock_actual numeric;
  v_stock_reintegrado integer := 0;
  v_lineas_aumentadas integer := 0; v_lineas_reducidas integer := 0; v_lineas_eliminadas integer := 0;
...
BEGIN
  ...
  IF jsonb_typeof(v_amenities) <> 'array' OR jsonb_array_length(v_amenities) = 0 THEN
    RETURN json_build_object('ok',true,'increments','[]'::jsonb,'mermas_creadas',0,'lineas_aumentadas',0,'lineas_reducidas',0,'lineas_eliminadas',0);
  END IF;
  ...
  FOR v_amenity IN SELECT * FROM jsonb_array_elements(v_amenities) LOOP
    ...
    IF v_delta > 0 THEN
      FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria, i.nombre AS insumo_nombre FROM public.recetas r JOIN public.insumos i ON i.id=r.insumo_id WHERE r.producto_id=v_producto_id ORDER BY r.insumo_id LOOP
        SELECT stock_actual INTO v_stock_actual FROM public.insumos WHERE id=v_receta.insumo_id FOR UPDATE;
        IF v_stock_actual < (v_receta.cantidad_necesaria * v_delta) THEN
          RAISE EXCEPTION 'Stock insuficiente de "%" para recalcular amenities: disponible %, requerido %', v_receta.insumo_nombre, v_stock_actual, v_receta.cantidad_necesaria*v_delta;
        END IF;
      END LOOP;
      FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=v_producto_id ORDER BY r.insumo_id LOOP
        UPDATE public.insumos SET stock_actual=stock_actual-(v_receta.cantidad_necesaria*v_delta) WHERE id=v_receta.insumo_id;
      END LOOP;
      IF v_existing_id IS NOT NULL THEN
        UPDATE public.detalle_ventas SET cantidad=v_new_qty, subtotal=0 WHERE id=v_existing_id;
      ELSE
        INSERT INTO public.detalle_ventas (venta_id, producto_id, cantidad, precio_unitario, subtotal, tipo_concepto, coworking_session_id)
        VALUES (NULL, v_producto_id, v_new_qty, 0, 0, 'amenity'::tipo_concepto, p_session_id);
      END IF;
      v_lineas_aumentadas := v_lineas_aumentadas + 1;
      v_increments := v_increments || jsonb_build_array(jsonb_build_object('producto_id',v_producto_id,'nombre',v_nombre,'cantidad',v_delta));
      ...
      IF v_requiere_prep IS DISTINCT FROM false THEN
        v_kds_items_pending := v_kds_items_pending || jsonb_build_array(jsonb_build_object('producto_id',v_producto_id,'nombre',v_nombre,'cantidad',v_delta));
      END IF;
    ELSIF v_delta < 0 AND v_existing_id IS NOT NULL THEN
      FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=v_producto_id ORDER BY r.insumo_id LOOP
        INSERT INTO public.mermas (insumo_id, cantidad, motivo, usuario_id, fecha_descuento)
        VALUES (v_receta.insumo_id, v_receta.cantidad_necesaria*abs(v_delta),
          format('Recalc amenities por baja de pax — %s ×%s (sesión %s, %s pax → %s pax)', v_nombre, abs(v_delta), v_session.cliente_nombre, v_session.pax_count, p_new_pax),
  ...
  IF jsonb_array_length(v_kds_items_pending) > 0 THEN
    ...
    SELECT v_kds_order_id, (k->>'producto_id')::uuid,
      ...
      (k->>'cantidad')::integer, NULL
    FROM jsonb_array_elements(v_kds_items_pending) AS k;
  END IF;

  INSERT INTO public.audit_logs (user_id, accion, descripcion, metadata)
  VALUES (v_user,'recalcular_amenities_pax',
    format('Recalc amenities por cambio de pax (sesión %s, %s → %s pax)', v_session.cliente_nombre, v_session.pax_count, p_new_pax),
    jsonb_build_object('session_id',p_session_id,'old_pax',v_session.pax_count,'new_pax',p_new_pax,
      'increments',v_increments,'mermas_creadas',v_total_mermas,'lineas_aumentadas',v_lineas_aumentadas,
      'lineas_reducidas',v_lineas_reducidas,'lineas_eliminadas',v_lineas_eliminadas,'kds_folio',v_kds_folio,'transaccional',true));

  RETURN json_build_object('ok',true,'increments',v_increments,'mermas_creadas',v_total_mermas,
    'lineas_aumentadas',v_lineas_aumentadas,'lineas_reducidas',v_lineas_reducidas,'lineas_eliminadas',v_lineas_eliminadas,'kds_folio',v_kds_folio);
END;
$$;

GRANT EXECUTE ON FUNCTION public.recalcular_amenities_pax(uuid, integer) TO authenticated;

-- cancelar_sesion_coworking redefinición final con fecha_descuento
CREATE OR REPLACE FUNCTION public.cancelar_sesion_coworking(
...
DECLARE
  v_user_id uuid := auth.uid();
  v_session RECORD; v_item jsonb; v_receta RECORD;
  v_cant_descontar numeric;
  v_mermas_creadas integer := 0; v_total_entregados integer := 0;
  v_stock_reintegrado integer := 0;
  v_descripcion_audit text; v_solicitante_id uuid;
  v_dv RECORD; v_comp RECORD;
  v_delivered_qty numeric; v_cant_reintegrar numeric;
  v_dv_created_at timestamp with time zone;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'No autenticado' USING ERRCODE='28000'; END IF;
  SELECT * INTO v_session FROM public.coworking_sessions WHERE id=p_session_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Sesión no encontrada'; END IF;
  IF v_session.estado <> 'activo' THEN RAISE EXCEPTION 'Solo se pueden cancelar sesiones activas (estado actual: %)', v_session.estado; END IF;
  IF p_is_admin THEN
    IF NOT public.has_role(v_user_id,'administrador') THEN RAISE EXCEPTION 'Acción restringida a administradores' USING ERRCODE='42501'; END IF;
  ELSE
    IF v_session.usuario_id <> v_user_id AND NOT public.has_role(v_user_id,'administrador') THEN
      RAISE EXCEPTION 'No tienes permiso para cancelar esta sesión' USING ERRCODE='42501';
    END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_entregados,'[]'::jsonb)) LOOP
    ...
    IF NULLIF(v_item->>'id','') IS NOT NULL THEN
      SELECT created_at INTO v_dv_created_at FROM public.detalle_ventas WHERE id=(v_item->>'id')::uuid;
    END IF;
        ...
        FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=v_comp.pid LOOP
          ...
          VALUES (v_receta.insumo_id, v_cant_descontar,
            format('Entrega paquete en sesión cancelada — %s (%s · %s ×%s)',
              v_session.cliente_nombre, COALESCE(v_item->>'nombre','paquete'),
              (SELECT nombre FROM public.productos WHERE id=v_comp.pid), (v_item->>'cantidad')),
            v_user_id, COALESCE(v_dv_created_at, now()));
          v_mermas_creadas := v_mermas_creadas + 1;
        END LOOP;
      END LOOP;
    ELSIF NULLIF(v_item->>'producto_id','') IS NOT NULL THEN
      FOR v_receta IN SELECT r.insumo_id, r.cantidad_necesaria FROM public.recetas r WHERE r.producto_id=(v_item->>'producto_id')::uuid LOOP
        ...
        INSERT INTO public.mermas (insumo_id, cantidad, motivo, usuario_id, fecha_descuento)
        VALUES (v_receta.insumo_id, v_cant_descontar,
          format('Entrega en sesión cancelada — %s (%s ×%s)', v_session.cliente_nombre, COALESCE(v_item->>'nombre','producto'), v_item->>'cantidad'),
          v_user_id, COALESCE(v_dv_created_at, now()));
        v_mermas_creadas := v_mermas_creadas + 1;
      END LOOP;
    END IF;
  ...
  FOR v_dv IN SELECT id, producto_id, cantidad, paquete_id, tipo_concepto, created_at FROM public.detalle_ventas WHERE coworking_session_id=p_session_id AND venta_id IS NULL ORDER BY id LOOP
    v_delivered_qty := 0;
    FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_entregados,'[]'::jsonb)) LOOP
      IF NULLIF(v_item->>'id','') = v_dv.id::text THEN v_delivered_qty := (v_item->>'cantidad')::numeric; EXIT; END IF;
    END LOOP;
    v_cant_reintegrar := v_dv.cantidad - v_delivered_qty;
    IF v_cant_reintegrar > 0 THEN
      IF v_dv.paquete_id IS NOT NULL THEN
